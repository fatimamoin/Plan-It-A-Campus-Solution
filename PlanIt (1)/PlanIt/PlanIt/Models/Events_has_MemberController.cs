    ...
    public class Events_has_MemberController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "idEvents_has_Member,Club_member_idClub_members,Events_idEvents, Why")] Events_has_Member events_has_Member)
        {
            if (ModelState.IsValid)
            {
                ...
                var old = db.Events.FirstOrDefault(x => x.idEvents == events_has_Member.Events_idEvents).seats;
                int seat_new = Events_has_Member.seats(System.Convert.ToInt32(old));
                db.Events.FirstOrDefault(x => x.idEvents == events_has_Member.Events_idEvents).seats = seat_new;
                db.SaveChanges();
